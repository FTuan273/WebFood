...
exports.getMyRestaurant = async (req, res) => {
  try {
    ...
    if (!r) return res.json({ success: true, restaurant: null, message: 'Bạn chưa có quán' });
...
exports.getProducts = async (req, res) => {
  try {
    const r = await getRestaurant(req.user.id);
    if (!r) return res.json({ success: true, products: [], message: 'Bạn chưa có quán' });
...
exports.getOrders = async (req, res) => {
  try {
    const r = await getRestaurant(req.user.id);
    if (!r) return res.json({ success: true, orders: [], message: 'Bạn chưa có quán' });
...
exports.getStats = async (req, res) => {
  try {
    const r = await getRestaurant(req.user.id);
    if (!r) return res.json({ success: true, stats: { totalOrders: 0, totalRevenue: 0, pending: 0, completed: 0, cancelled: 0, totalProducts: 0, last7: [] }, message: 'Bạn chưa có quán' });
